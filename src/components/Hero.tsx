...
const Typewriter: React.FC<{ words: string[] }> = ({ words }) => {
    ...
    const handleTyping = () => {
      ...
      if (!isDeleting) {
        ...
        if (currentText.length + 1 === fullWord.length) {
          setIsDeleting(true);
          setTypingSpeed(3000); // 3 seconds pause when fully typed
        } else {
          setTypingSpeed(100);  // 100ms typing speed
        }
      } else {
        setCurrentText(fullWord.slice(0, currentText.length - 1));
        if (currentText.length === 0) {
          ...
          setTypingSpeed(800);  // 800ms delay before starting next word
        } else {
          setTypingSpeed(50);   // 50ms deleting speed
        }
      }
    };
  ...
  return (
    <span className="border-r-2 border-amber-500 pr-1 select-none">
      {currentText}
    </span>
  );
};
